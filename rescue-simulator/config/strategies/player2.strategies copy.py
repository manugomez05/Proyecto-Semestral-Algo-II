...
class Strategy2:
    ...
    - Resto de vehículos: Dijkstra para recursos
    """
    ...
    def dijkstra_shortest_path(self, start: Tuple[int, int], target: Tuple[int, int]) -> Optional[List[Tuple[int, int]]]:
        """
        Implementa algoritmo de Dijkstra para encontrar el camino más corto.
        Evita minas y vehículos enemigos.
        """
        rows = self.map.rows if hasattr(self.map, "rows") else self.map_height
        cols = self.map.cols if hasattr(self.map, "cols") else self.map_width
        ...
        while pq:
            ...
            for dr, dc in directions:
                ...
                # Verificar si está minada
                if not self.is_position_safe(new_row, new_col):
                    continue
                
                # Verificar si hay vehículo enemigo
                node = self.map.graph.get_node(new_row, new_col)
                if node and node.state == "vehicle" and node.content:
                    # Verificar si es enemigo (simplificado: asumir que si no es nuestro, es enemigo)
                    continue
    ...
    def find_best_resource_dijkstra(self, vehicle, resources: List) -> Optional[Tuple[object, Tuple[int, int], List[Tuple[int, int]]]]:
        """
        Encuentra el mejor recurso usando Dijkstra.
        Retorna (recurso, posición, camino) o None
        """
        if not resources:
            return None
        ...
        for resource in resources:
            ...
            path = self.dijkstra_shortest_path(vehicle_pos, (res_row, res_col))
            
            if path and len(path) > 1:
                # Score: valor / (longitud del camino)
                path_length = len(path) - 1
                score = value / path_length if path_length > 0 else value
        ...
        if best_resource and best_path:
            # Obtener posición del mejor recurso
            if hasattr(best_resource, "position"):
                res_pos = best_resource.position
            else:
                res_pos = best_resource.get("position")
            
            if res_pos and len(res_pos) == 2:
                res_x, res_y = res_pos[0], res_pos[1]
                res_row, res_col = res_y, res_x
                return (best_resource, (res_row, res_col), best_path)
        ...
        for vehicle_id, vehicle in vehicles.items():
            ...
            if getattr(vehicle, "status", None) == "need_return":
                ...
                if vehicle_type == "moto":
                    ...
                    if closest_truck:
                        ...
                        # Si no se movió (está en la misma posición), usar Dijkstra para recursos
                        if new_row == row and new_col == col:
                            best_resource = self.find_best_resource_dijkstra(vehicle, resources)
                            if best_resource:
                                _, res_pos, path = best_resource
                                if path and len(path) > 1:
                                    next_step = path[1]
                                    if not self.is_occupied_by_teammate(next_step[0], next_step[1], player, getattr(vehicle, "id", None)):
                        ...
                        # No hay camiones válidos, usar Dijkstra para recursos
                        best_resource = self.find_best_resource_dijkstra(vehicle, resources)
                        if best_resource:
                            _, res_pos, path = best_resource
                            if path and len(path) > 1:
                                # Verificar que el siguiente paso no esté ocupado por compañero
                                next_step = path[1]
                                if not self.is_occupied_by_teammate(next_step[0], next_step[1], player, getattr(vehicle, "id", None)):
                                    new_row, new_col = next_step
                                else:
                                    # Si está ocupado, usar move_towards_target como fallback
                                    new_row, new_col = self.move_towards_target(vehicle, res_pos, player)
                            else:
                                new_row, new_col = self.move_towards_target(vehicle, res_pos, player)
                        else:
            ...
            try:
                ...
                if hasattr(self.map, "graph") and callable(getattr(self.map.graph, "place_vehicle", None)):
                    placed = self.map.graph.place_vehicle(vehicle, new_row, new_col, tick=tick, mine_manager=mine_manager, player1=self.enemy_player, player2=player)
                elif callable(getattr(self.map, "place_vehicle", None)):
                    placed = self.map.place_vehicle(vehicle, new_row, new_col, tick=tick, mine_manager=mine_manager, player1=self.enemy_player, player2=player)
            ...
            if not placed or (new_row == row and new_col == col):
                ...
                for dr, dc in alternatives:
                    ...
                    if self.is_position_safe(alt_row, alt_col) and not self.is_occupied_by_teammate(alt_row, alt_col, player, getattr(vehicle, "id", None)):
                        try:
                            if hasattr(self.map, "graph") and callable(getattr(self.map.graph, "place_vehicle", None)):
                                placed = self.map.graph.place_vehicle(vehicle, alt_row, alt_col, tick=tick, mine_manager=mine_manager, player1=self.enemy_player, player2=player)
                            elif callable(getattr(self.map, "place_vehicle", None)):
                                placed = self.map.place_vehicle(vehicle, alt_row, alt_col, tick=tick, mine_manager=mine_manager, player1=self.enemy_player, player2=player)

...
class Strategy1:
    ...
    def find_best_resource(self, vehicle, resources: List, player, vehicle_id: str, planned_moves: Dict[str, Tuple[int, int]] = None, assigned_targets: Set[Tuple[int, int]] = None) -> Optional[Tuple[object, Tuple[int, int]]]:
        ...
        OPTIMIZADO: usa heurística Manhattan para priorizar recursos y reducir BFS innecesarios
        """
        if not resources:
            return None
        ...
        for value in sorted(candidates_by_value.keys(), reverse=True):
            ...
            # Solo hacer BFS en los 3 más cercanos según Manhattan de este valor
            max_bfs_checks = min(3, len(candidates))
            
            for candidate in candidates[:max_bfs_checks]:
                res_pos = candidate['pos']
                
                # Calcular distancia usando BFS
                distance = self.bfs_distance(vehicle_pos, res_pos, player, vehicle_id, planned_moves)
    ...
    def find_closest_enemy_truck(self, vehicle, enemy_trucks: List[Tuple[int, int]], player, vehicle_id: str, planned_moves: Dict[str, Tuple[int, int]] = None, assigned_targets: Set[Tuple[int, int]] = None) -> Optional[Tuple[int, int]]:
        ...
        OPTIMIZADO: usa Manhattan para priorizar y reduce BFS
        """
        if not enemy_trucks:
            return None
        ...
        # Solo hacer BFS en los 2 más cercanos según Manhattan
